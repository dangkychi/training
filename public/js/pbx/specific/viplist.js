...
function changeDestination(type, dest = '') {
    $("#destination").closest("div.form-group").removeClass("error");
    $("#destination").parent().parent().find(".help-block").remove();
    $("#destination").parent().show();
    $.ajax({
        url: base_url + "specific/viplist/destination/" + type,
        type: "GET",
        dataType: 'json',
...
function activeSidebar(data = false) {
    ...
    if (data != false && data.formData.destinatype !== "off" && data.formData.destinatype !== "phone") {
        changeDestination(data.formData.destinatype, data.formData.destination);
    } else {
        changeDestination("internal");
    }

    $("#destinatype,#group").select2({ dropdownAutoWidth: true, width: '100%', minimumResultsForSearch: Infinity });
    ...
    $("#frmSubmit").on("click", function () {
        ...
        let data = $("#frm_viplist").serializeObject(),
            url = base_url + (typeof data.id === "undefined" ? "specific/viplist/add" : "specific/viplist/update");
...
$(document).ready(function () {
    ...
    var grid = $("#grid").kendoGrid({
        dataSource: {
            transport: {
                read: {
                    url: base_url + "specific/viplist/lists",
                ...
                parameterMap: function (options, operation) {
                    ...
                    if (options.filter != undefined) {
                        options.filter.filters.forEach(function (item) {
                            console.log(item)
                            if (item.field === "viplist_groupname") item.field = "group";
                        });
                    }
                    return options;
                }
        ...
        columns: [{
            ...
            field: "number",
            title: controlsKendo.grid.column.title.number,
            width: 170,
            headerAttributes: {
                "class": "table-header-cell",
                ...
                cell: {
                    ...
                    template: function (args) {
                        args.element.kendoDropDownList({
                            dataSource: [
                                { id: "1", text: controlsKendo.grid.filter.value.enable },
                                { id: "0", text: controlsKendo.grid.filter.value.disable }
                            ],
                            filter: "startswith",
                            optionLabel: "--Select--",
                            dataTextField: "text",
            ...
            field: "viplist_groupname",
            title: controlsKendo.grid.column.title.group,
            width: 140,
            filterable: {
                cell: {
                    showOperators: false,
                    template: function (args) {
                        args.element.kendoDropDownList({
                            dataSource: {
                                ...
                                transport: {
                                    read: {
                                        url: base_url + "specific/viplist/groups",
                                        contentType: "application/json",
                                        type: "GET"
                                    }
                                },
                            ...
                            optionLabel: "--Select--",
                            dataTextField: "viplist_groupname",
                            dataValueField: "id",
                            valuePrimitive: true
                        });
                    }
            ...
            field: "note",
            title: controlsKendo.grid.column.title.note
        }, {
            field: "cretime",
            ...
            command: [{
                ...
                click(e) {
                    ...
                    $(".k-button-yes").click(function () {
                        ...
                        let csrf = $('meta[name=csrf]').attr('content').split(":"),
                            post = { [csrf[0]]: csrf[1], id: formData.id, number: formData.number, customer_code: formData.customer_code };
                        $.post(base_url + "specific/viplist/destroy", post, function (response, status) {
