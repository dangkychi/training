...
function activeSidebar(formData = false) {
    ...
    $("#frmSubmit").on("click", function () {
        ...
        let data = $("#frm_staff").serializeObject(),
            url = base_url + (typeof data.id === "undefined" ? "manage/staff/add" : "manage/staff/update");
...
$(document).ready(function () {
    ...
    var grid = $("#grid").kendoGrid({
        dataSource: {
            transport: {
                read: {
                    url: base_url + "manage/staff/lists",
        ...
        columns: [{
            ...
            field: "role",
            title: controlsKendo.grid.column.title.role,
            filterable: false,
            width: 100,
            headerAttributes: {
                class: "table-header-cell",
            ...
            command: [{
                ...
                template: $("#gridActionEditTemplate").html(),
                click(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    if ((curentUserInfo.role == "admin" && formData.role == "sadmin") || (curentUserInfo.role == formData.role && curentUserInfo.id > formData.id)) {
                        toastr.error(controlsKendo.alert.role_denied, 'Action Denied!', { positionClass: 'toast-top-left', containerId: 'toast-top-left', "showMethod": "slideDown", "hideMethod": "slideUp", timeOut: 3000 });
                        return;
                    }
                    var template = kendo.template($("#addnewTemplate").html());
                ...
                click(e) {
                    ...
                    if ((curentUserInfo.role == "admin" && formData.role == "sadmin") || (curentUserInfo.role == formData.role && curentUserInfo.id > formData.id)) {
                        toastr.error(controlsKendo.alert.role_denied, 'Action Denied!', { positionClass: 'toast-top-left', containerId: 'toast-top-left', "showMethod": "slideDown", "hideMethod": "slideUp", timeOut: 4000 });
                        return;
                    }
                    ...
                    $(".k-button-yes").click(function () {
                        ...
                        $.post(base_url + "manage/staff/destroy", post, function (response, status) {
