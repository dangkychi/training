/*=========================================================================================
    File Name: usergroup.js
    Description: Usergroup Table
    --------------------------------------------------------------------------------------
...
function activeSidebar(formData = false) {
    ...
    var $lcr_name = $('#lcr_name');
    $lcr_name.select2({
        dropdownAutoWidth: false,
        width: '100%',
        ajax: {
            url: base_url + 'settings/usergroup/lcrs',
            dataType: 'json',
            type: "GET",
            delay: 250,
    ...
    if (formData != false) {
        $lcr_name.select2("trigger", "select", { data: { id: formData.lcrgroup, text: formData.lcr_name } });
    ...
    $("#frmSubmit").on("click", function () {
        ...
        let data = $("#frm_usergroup").serializeObject(),
            url = base_url + (typeof data.id === "undefined" ? "settings/usergroup/add" : "settings/usergroup/update");
...
//Open sidebar 
(function () {
    $("#add-usergroup").on("click", function () {
...
$(document).ready(function () {
    ...
    var grid = $("#grid").kendoGrid({
        dataSource: {
            transport: {
                read: {
                    url: base_url + "settings/usergroup/lists",
        ...
        columns: [{
            ...
            field: "lcr_name",
            title: controlsKendo.grid.column.title.lcr_name,
            width: 200,
            headerAttributes: {
                "class": "table-header-cell",
            ...
            field: "nDNC",
            title: controlsKendo.grid.column.title.nDNC,
            width: 160,
                ...
                cell: {
                    ...
                    template: function (args) {
                        args.element.kendoDropDownList({
                            dataSource: [
                                { id: "1", text: controlsKendo.grid.filter.value.use_nDNC },
                                { id: "0", text: controlsKendo.grid.filter.value.not_use_nDNC }
                            ],
                            optionLabel: "--Select--",
                            dataTextField: "text",
                            dataValueField: "id",
            ...
            field: "callout_redirect",
            title: controlsKendo.grid.column.title.callout_redirect,
            width: 130,
            template: function (dataItem) {
                if (dataItem.nDNC == 1) {
                    return `<button class="btn-icon btn btn-primary btn-round btn-sm" type="button" data-toggle="tooltip" data-placement="top" data-original-title="In Use"><i class="feather icon-check-circle"></i></button>`;
            ...
            field: "groupnote",
            title: controlsKendo.grid.column.title.groupnote
        }, {
            field: "cretime",
                ...
                click(e) {
                    ...
                    $(".k-button-yes").click(function () {
                        ...
                        let csrf = $('meta[name=csrf]').attr('content').split(":"),
                            post = { [csrf[0]]: csrf[1], id: formData.id, groupid: formData.groupid, groupname: formData.groupname, customer_code: formData.customer_code };
                        $.post(base_url + "settings/usergroup/destroy", post, function (response, status) {
