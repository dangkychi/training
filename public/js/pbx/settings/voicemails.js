...
function changeDestination(type, dest = '') {
    $.ajax({
        url: base_url + "settings/voicemails/destination/" + type,
        type: "GET",
        dataType: 'json',
...
function activeSidebar(data = false) {
    ...
    $("#frmSubmit").on("click", function () {
        ...
        let data = $("#frm_voicemail").serializeObject(),
            url = base_url + (typeof data.id === "undefined" ? "settings/voicemails/add" : "settings/voicemails/update");
...
//Open sidebar 
(function () {
    $("#add-voicemail").on("click", function () {
    ...
    $("#sync_config").click(function () {
        ...
        let csrf = $('meta[name=csrf]').attr('content').split(":"),
            post = { [csrf[0]]: csrf[1] };
        $.post(base_url + "settings/voicemails/sync_config", post, function (response, status) {
...
$(document).ready(function () {
    ...
    var grid = $("#grid").kendoGrid({
        dataSource: {
            transport: {
                read: {
                    url: base_url + "settings/voicemails/lists",
        ...
        columns: [{
            ...
            field: "pin",
            title: controlsKendo.grid.column.title.pin,
            width: 150,
            headerAttributes: {
                "class": "table-header-cell",
                style: "text-align: center; font-size: 14px"
            ...
            field: "full_name",
            title: controlsKendo.grid.column.title.full_name,
            width: 180
        }, {
            field: "email_address",
            title: controlsKendo.grid.column.title.email_address
        }, {
            field: "short_email",
            title: controlsKendo.grid.column.title.short_email,
            filterable: false
        }, {
            attributes: {
                class: "table-cell",
            ...
            command: [{
                ...
                click(e) {
                    ...
                    $(".k-button-yes").click(function () {
                        ...
                        let csrf = $('meta[name=csrf]').attr('content').split(":"),
                            post = { [csrf[0]]: csrf[1], id: formData.id, mailbox: formData.mailbox, customer_code: formData.customer_code };
                        $.post(base_url + "settings/voicemails/destroy", post, function (response, status) {
