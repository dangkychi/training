/*=========================================================================================
    File Name: sms_configs.js
    Description: SMS Configs Table
    --------------------------------------------------------------------------------------
...
function activeSidebar(formData = false) {
    ...
    $brand_id.select2({
        ...
        width: '100%',
        ajax: {
            url: base_url + 'settings/sms_configs/brandname',
            dataType: 'json',
            type: "GET",
            delay: 250,
    ...
    $brand_id.on('select2:select', function (evt) {
        if (evt) {
    ...
    $last_action.select2({
        ...
        width: '100%',
        ajax: {
            url: base_url + 'settings/sms_configs/ivrmenu',
            dataType: 'json',
            type: "GET",
            delay: 250,
            data: function (params) {
    ...
    $last_action.on('select2:select', function (evt) {
        if (evt) {
            $(evt.target).closest("div.form-group").removeClass("error");
            $(evt.target).parent().parent().find(".help-block").remove();
        }
    ...
    if (formData != false) {
        $brand_id.select2("trigger", "select", { data: { id: formData.brand_id, text: formData.brandname } });
        if (formData.last_action !== "hangup") {
            $.get(base_url + 'settings/sms_configs/ivrmenubyid?id=' + formData.last_action, function (res) {
                if (res.total > 0) {
                    $last_action.select2("trigger", "select", { data: { id: formData.last_action, text: res.data.name } });
    ...
    $("#frmSubmit").on("click", function () {
        ...
        let data = $("#frm_sms_template").serializeObject(),
            url = base_url + (typeof data.id === "undefined" ? "settings/sms_configs/add_template" : "settings/sms_configs/update_template");
...
function showSMSConfigs() {
    var windowSMSConfigs = $("#windowSMSConfigs").kendoWindow({
        title: "SMS Configs",
        visible: false, //the window will not appear before its .open method is called
        width: (window.innerWidth - 50) + "px",
        height: (window.outerHeight - 260) + "px",
        close: function (e) {
            $('#grid_sms_configs').empty();
        }
    }).data("kendoWindow");

    var grid_sms_configs = $("#grid_sms_configs").kendoGrid({
        dataSource: {
            transport: {
                read: {
                    url: base_url + "settings/sms_configs/config_lists",
                    contentType: "application/json",
                    type: "GET"
                },
                create: {
                    url: base_url + "settings/sms_configs/add_config",
                    type: "POST",
                    complete: function (e) {
                        ...
                        $("#grid_sms_configs").data("kendoGrid").dataSource.read();
                    }
                },
                update: {
                    url: base_url + "settings/sms_configs/update_config",
                    type: "POST",
                    complete: function (e) {
                        let response = e.responseJSON;
                        if (response.status == 1) {
            ...
            schema: {
                ...
                model: {
                    id: "id",
                    fields: {
                        brandname: { editable: true, validation: { required: true } },
                        auth_key: { editable: true, validation: { required: true } }
                    }
                }
            },
            page: 1,
        ...
        columns: [
            {
                field: "brandname",
                title: controlsKendo.grid.column.title.brandname,
                width: 250,
                headerAttributes: {
                ...
                field: "auth_key",
                title: controlsKendo.grid.column.title.auth_key,
                headerAttributes: {
                    "class": "table-header-cell",
                    style: "text-align: center; font-size: 14px"
                },
                ...
                command: [{
                    ...
                    name: "delete-config",
                    template: kendo.template($("#gridActionDeleteConfigTemplate").html()),
                    click(e) {
                        e.preventDefault();
                        var windowConfirmTemplate = kendo.template($("#windowConfirmConfigTemplate").html());
                        ...
                        windowConfirm.title($("#windowConfirmConfigTemplate").attr("title"));
                        ...
                        $(".k-button-yes").click(function () {
                            windowConfirm.close();
                            kendo.ui.progress($("#grid_sms_configs").data("kendoGrid").element, true);
                            let csrf = $('meta[name=csrf]').attr('content').split(":"),
                                post = { [csrf[0]]: csrf[1], id: formData.id, brandname: formData.brandname, customer_code: formData.customer_code };
                            $.post(base_url + "settings/sms_configs/destroy_config", post, function (response, status) {
                                kendo.ui.progress($("#grid_sms_configs").data("kendoGrid").element, false);
    ...
    windowSMSConfigs.one("activate", function () {
        grid_sms_configs.resize();
    });
    windowSMSConfigs.center().open();
}
...
(function () {
    ...
    $("#show-sms-configs").on("click", function (e) {
        e.preventDefault();
        showSMSConfigs();
    });
})();
...
$(document).ready(function () {
    ...
    var grid = $("#grid").kendoGrid({
        dataSource: {
            transport: {
                read: {
                    url: base_url + "settings/sms_configs/template_lists",
                    contentType: "application/json",
                    type: "GET"
                }
            },
            ...
            command: [{
                ...
                click(e) {
                    ...
                    $(".k-button-yes").click(function () {
                        ...
                        $.post(base_url + "settings/sms_configs/destroy_template", post, function (response, status) {
